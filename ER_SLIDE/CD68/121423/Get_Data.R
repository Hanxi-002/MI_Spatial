...
AF_data <- subset(Geomx_V3, select = Geomx_V3@phenoData@data$`CD 68` == TRUE)
...
row.names(AF_mat)
old_x <- read.csv("/ix/djishnu/Hanxi/MI_Spatial/ER_SLIDE/CD68/022423/x.csv", row.names = 1)
sum(row.names(AF_mat) != row.names(old_x))

write.csv(AF_mat, '/ix/djishnu/Hanxi/MI_Spatial/ER_SLIDE/CD68/121423/Data/x.csv')
write.csv(y, '/ix/djishnu/Hanxi/MI_Spatial/ER_SLIDE/CD68/121423/Data/y.csv')
