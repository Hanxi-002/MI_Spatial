...
setwd("/ix/djishnu/Hanxi/MI_Spatial/ER_SLIDE/CD68/050123")
library(doParallel)
library(ggplot2)
library(SLIDE)
library(SLIDEHelper)
...
x_path <- "x.csv"
er_path <- "ER_Results/final_delta_0.01_lambda_0.5.rds"
out_path <- "SLIDE_Results/"

z_matrix <- CalcZMatrix(x_path, er_path, out_path)

y_path <- "y.csv"
SLIDE_res <- runSLIDE(y_path, z_path = NULL, z_matrix, er_path, do_interacts = TRUE, niter = 500, spec = 0.2)
