...
for s in annot["SlideName"].unique():
    ...
    for r in df_slide["ROILabel"].unique():
        ...
        if len(df_roi) == 3:
            ...
            dcc = df_roi[df_roi["SegmentLabel"] == "active fibroblast"]["DccNames"].values[0]
            print(resting_ratio)
            print(active_ratio)
...
y = y.dropna()
y.to_csv("/Users/xiaoh/Library/CloudStorage/OneDrive-UniversityofPittsburgh/MI_Spatial/ER_SLIDE/Within_Region/cross_prediction/AF_RF_082123/y.csv", index = False)
