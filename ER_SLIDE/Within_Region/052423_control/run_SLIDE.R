...
x_path <- "x.csv"
er_path <- "results/final_delta_0.1_lambda_0.5.rds"
...
num_top_feats <- 10
condition <- "auc"
Final_res <- GetTopFeatures(x_path, y_path, er_path, out_path, SLIDE_res, num_top_feats = 10, condition)

saveRDS(Final_res, "SLIDE_Result/SLIDE_res.rds")

plotSigGenes(Final_res, out_path, annotate_anchors = FALSE)