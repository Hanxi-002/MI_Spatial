...
z_mat['color'] = y$ccr2
z_mat$color <- ifelse(z_mat$color == 0, 'Close to CCR2- Macs', "Close to CCR2+ Macs")
...
p <- ggplot(z_mat, aes(x = Z37, y = Z17, color = color)) + 
  ...
  scale_color_manual(values = c("Close to CCR2- Macs" = "#d19999", "Close to CCR2+ Macs" = "#8b0000")) +
  theme_minimal() + 
  theme(
...
print(p)
ggsave("/ocean/projects/cis240075p/hxiao2/MI_Spatial/ER_SLIDE/Within_Region/091124_hf_CCR2/Results/0.01_0.5_out_final/Z37_Z17_scatter_20250414.pdf", plot = p, width = 4, height = 5, units = "in")
