...
os.chdir("/ocean/projects/cis240075p/hxiao2/MI_Spatial")
...
for i, cell in enumerate(adata.obs['cell_type'].unique()):
    ...
    sc.pl.umap(cell_adata, color='Status', title=cell, show=False, size = 100, ax=axes[i])


plt.tight_layout()
plt.savefig("Cell_Oracle/all_cell/012924/umap_cell_types.pdf", dpi=300)
plt.show()