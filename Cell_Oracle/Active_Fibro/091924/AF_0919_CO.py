...
# Load the data
print(os.getcwd())
os.chdir("/ix/djishnu/Hanxi/MI_Spatial/Cell_Oracle/Active_Fibro/091924")
...
#%% ###################################################################
'''
Repeat 021324 analysis to save the oracle object post perturbation with the adjusted n_neighbros parameter.
'''
...
for goi in overlap_TF_list:
    ...
    calc_cell_identity_shifts(oracle=adata_oracle.oracle, goi=goi, save_folder = save_folder, \
                              scale = 40, perturb_value = 0.0, n_neighbors = 10)

    get_optimal_min_mass(n_grid=40, oracle=adata_oracle.oracle, n_neighbors = 10)
    ...
    dill.dump(adata_oracle, open('toy' + goi + '_adata_oracle.pkl', 'wb'))
#%%
#%% ###################################################################
'''
